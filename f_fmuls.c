...
void F_FMULS(void){
    ...
    signed char R1 = ((getOpcode() & 0x0070) >> 4) + 16;   //identyfikacja numeru rejestru arg. 1
    signed char R2 = (getOpcode() & 0x0007) + 16;          //identyfikacja numeru rejestru arg. 2
    ...
    result = (((getRegister(R1) & 0x80) ^ (getRegister(R2) & 0x80)) == 0x80) ? (~result + 1) : (result);
